...
/// Layar verifikasi email via kode OTP 6-digit (PBI: konfirmasi email lintas
/// device — pengguna cukup membaca kode dari email manapun lalu mengetiknya
/// di sini, tanpa perlu membuka tautan deep-link di device yang sama.
...
class _VerifyEmailScreenState extends ConsumerState<VerifyEmailScreen> {
  ...
  Future<void> _verify(AppStrings s) async {
    if (!_formKey.currentState!.validate()) return;
    final ok = await ref.read(authProvider.notifier)
        .verifyEmailOtp(widget.email, _codeCtrl.text.trim());
    if (!mounted) return;
    if (!ok) {
      SnackbarUtil.showError(context, ref.read(authProvider).error ?? s.authVerifyFailed);
    }
  ...
  Future<void> _resend(AppStrings s) async {
    final ok = await ref.read(authProvider.notifier).resendEmailOtp(widget.email);
    if (!mounted) return;
    if (ok) {
    ...
    return Scaffold(backgroundColor: AppColors.background,
      body: SafeArea(child: SingleChildScrollView(
        ...
        child: Form(key: _formKey, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(s.authVerifyTitle, style: AppTextStyles.displayLg()),
          const SizedBox(height: 12),
          Text(s.authVerifyBody(widget.email), style: AppTextStyles.bodyMd(c: AppColors.textSecondary)),
          ...
          AppTextField(label: s.authVerifyCodeLabel, hint: s.authVerifyCodeHint, controller: _codeCtrl,
              ...
              onFieldSubmitted: (_) => _verify(s), validator: s.validateOtpCode),
          const SizedBox(height: 32),
          AppButton(label: s.authVerifyButton, onPressed: isLoading ? null : () => _verify(s), isLoading: isLoading),
          const SizedBox(height: 24),
